...
function receiptPngCanvas(transactionId){
  ...
  // PNG WhatsApp intentionally mirrors the thermal-printer receipt: no banner,
  // logo, colours, or e-receipt-only labels.
  const scale = 3;
  const width = 600;
  const padding = 44;
  ...
  const fontSize = 21;
  const lineHeight = 30;
  const font = (bold=false) => `${bold ? "700" : "400"} ${fontSize}px "Courier New", monospace`;
  ...
  const addGap = (size=8) => rows.push({ type:"gap", size });
  const addRow = (left, right, options={}) => rows.push({ type:"row", left:String(left || ""), right:String(right || ""), ...options });

  addText("MATCHA BEE x DAILY KOPI", { align:"center", bold:true });
  addText("TO GO", { align:"center", bold:true });
  addGap(4);
  addText("Kasir & Penjualan", { align:"center" });
  addGap(10);
  addRule();
  addGap(10);
  addRow("No. Transaksi:", transactionId, { boldRight:true });
  addGap(8);
  addRow("Tanggal:", formatDateTimeDMY(first.date), { boldRight:true });
  addGap(8);
  addRow("Metode Bayar:", salePayment(first), { boldRight:true });
  addGap(10);
  addRule();
  addGap(10);
  ...
  items.forEach((item, index) => {
    addText(`${index + 1}. ${item.productName}`, { bold:true });
    addRow(`${item.qty} x ${rupiah(item.sellPrice)}`, rupiah(Number(item.sellPrice || 0) * Number(item.qty || 0)));
    if(item.note) addText(`Catatan: ${item.note}`, { small:true });
    addGap(8);
  });
  addRule();
  addGap(10);
  ...
  addRule();
  addGap(10);
  addRow("TOTAL", rupiah(total), { bold:true });
  addGap(10);
  addRule();
  addGap(11);
  addText("Terima kasih sudah membeli \uD83D\uDE4F", { align:"center" });
  addText("\u2615 Waktunya #Matcha Bee x Daily", { align:"center" });
  addText("Kopi To Go", { align:"center" });
  addGap(4);
  addText("\uD83D\uDCF7 IG: @matcha_dailytogo", { align:"center" });
  addGap(12);

  const wrap = (text, bold=false, small=false) => {
    measure.font = small ? `400 18px "Courier New", monospace` : font(bold);
    return wrapCanvasText(measure, text, contentWidth);
  };
  const rowHeight = row => {
    if(row.type === "gap") return row.size;
    if(row.type === "rule") return 1;
    if(row.type === "row") return lineHeight;
    const size = row.small ? 25 : lineHeight;
    return wrap(row.text, row.bold, row.small).length * size;
  };
  const height = rows.reduce((sum, row) => sum + rowHeight(row), padding * 0.75) + padding * 0.75;
  ...
  let y = padding * 0.75;
  rows.forEach(row => {
    if(row.type === "gap") { y += row.size; return; }
    if(row.type === "rule") {
      ctx.save();
      ctx.strokeStyle = "#000";
      ctx.lineWidth = 1;
      ctx.setLineDash([5, 4]);
      ctx.beginPath(); ctx.moveTo(padding, y + .5); ctx.lineTo(width - padding, y + .5); ctx.stroke();
      ctx.restore();
      y += 1;
      return;
    }
    if(row.type === "row") {
      ctx.font = font(row.bold || false);
      ctx.fillStyle = "#000";
      ctx.fillText(row.left, padding, y);
      ctx.font = font(row.boldRight || row.bold || false);
      const rightWidth = ctx.measureText(row.right).width;
      ctx.fillText(row.right, width - padding - rightWidth, y);
      y += lineHeight;
      return;
    }
    ctx.font = row.small ? `400 18px "Courier New", monospace` : font(row.bold || false);
    ctx.fillStyle = "#000";
    const lines = wrap(row.text, row.bold, row.small);
    const heightPerLine = row.small ? 25 : lineHeight;
    lines.forEach(line => {
